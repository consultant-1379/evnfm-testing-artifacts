...
*/}}
{{- define "eric-data-document-database-pg.hkln.name" -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.job-inventory-secret-name" -}}
{{ printf "%s-inventory" (include "eric-data-document-database-pg.hkln.name" .) }}
{{- end }}

{{/*
Name to be used for hooklauncher job container.
*/}}
{{- define "eric-data-document-database-pg.hkln.containerName" -}}
{{ printf "hooklauncher" }}
{{- end -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.image-path" }}
...
Create version
*/}}
{{- define "eric-data-document-database-pg.hkln.version" -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.helm-labels" }}
chart: {{ .Chart.Name }}
release: "{{ .Release.Name }}"
app: {{ template "eric-data-document-database-pg.hkln.name" . }}
app.kubernetes.io/name: {{ template "eric-data-document-database-pg.hkln.name" . }}
app.kubernetes.io/version: {{ template "eric-data-document-database-pg.hkln.version" . }}
...
Common labels
*/}}
{{- define "eric-data-document-database-pg.hkln.labels" -}}
  {{- $helmLabels := include "eric-data-document-database-pg.hkln.helm-labels" . | fromYaml -}}
  {{- $globalLabels := (.Values.global).labels -}}
  {{- $serviceLabels := .Values.labels -}}
  {{- $staticLabels := dict -}}
  {{- $_ := set $staticLabels "sidecar.istio.io/inject" "false" -}}
  {{- include "eric-data-document-database-pg.mergeLabels" (dict "location" .Template.Name "sources" (list $helmLabels $globalLabels $serviceLabels $staticLabels)) | trim }}
{{- end -}}

{{/*
Create annotation for the product information
*/}}
{{- define "eric-data-document-database-pg.hkln.product-info" }}
...
Common annotations
*/}}
{{- define "eric-data-document-database-pg.hkln.annotations" -}}
  {{- $productInfoAnn := include "eric-data-document-database-pg.hkln.product-info" . | fromYaml -}}
  {{- $globalAnn := (.Values.global).annotations -}}
  {{- $serviceAnn := .Values.annotations -}}
  {{- include "eric-data-document-database-pg.mergeAnnotations" (dict "location" .Template.Name "sources" (list $productInfoAnn $globalAnn $serviceAnn)) | trim }}
{{- end -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.cleanup" -}}
{{- $cleanup := "true" -}}
...
Default value: true
*/}}
{{- define "eric-data-document-database-pg.hkln.terminateEarlyOnFailure" -}}
...
Default value: 6
*/}}
{{- define "eric-data-document-database-pg.hkln.backoffLimit" -}}
{{- $backoffLimit := 6 -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.resources" -}}
  {{- $limitsDict := dict -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.imagePullPolicy" -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.pullSecrets" -}}
{{- $pullSecret := "" -}}
{{- if .Values.global -}}
...
Create tolerations
*/}}
{{- define "eric-data-document-database-pg.hkln.tolerations" -}}
  {{- block "eric-data-document-database-pg.merge-tolerations" (dict "root" . "podbasename" "hooklauncher") -}}
    {{- with .root -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.priorityClassName" -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.profile-helper" -}}
  {{- $profileType := "" -}}
  {{- $localhostProfile := "" -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.appArmorProfileAnn" -}}
  {{- $result := include "eric-data-document-database-pg.hkln.profile-helper" (dict "root" .root "profileKey" "appArmorProfile" "localhostkey" "localhost" "containerName" .containerName) | fromYaml -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.seccompProfile" -}}
{{- if eq .Scope "Pod" -}}
{{- include "eric-data-document-database-pg.hkln.profile-helper" (dict "root" .root "profileKey" "seccompProfile" "localhostkey" "Localhost" "containerName" "") -}}
{{- else -}}
{{- include "eric-data-document-database-pg.hkln.profile-helper" (dict "root" .root "profileKey" "seccompProfile" "localhostkey" "Localhost" "containerName" .Scope) -}}
{{- end -}}
{{- end -}}

{{- define "eric-data-document-database-pg.hkln.securityPolicy.reference" -}}
  {{- if .Values.global -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.securityPolicy.rolename" -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.securityPolicy.rolekind" -}}
{{- $rolekind := "" -}}
  {{- if .Values.global -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.securityPolicy-rolebinding-name" -}}
{{- $rolename := include "eric-data-document-database-pg.hkln.securityPolicy.rolename" .shh -}}
{{- $rolecipher := substr 0 1 (include "eric-data-document-database-pg.hkln.securityPolicy.rolekind" .shh) | lower -}}
{{- $hklnname := include "eric-data-document-database-pg.hkln.name" .top -}}
...
Create timezone
*/}}
{{- define "eric-data-document-database-pg.hkln.timezone" -}}
{{- $timezone := "UTC" -}}
{{- if .Values.global -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.nodeSelector" -}}
  {{- $hkln := (.Values.nodeSelector).hooklauncher -}}
  {{- $global := (.Values.global).nodeSelector -}}
  {{- include "eric-data-document-database-pg.aggregatedMerge" (dict "context" "nodeSelector" "location" .Template.Name "sources" (list $global $hkln)) -}}
{{- end -}}

{{/*
Integration charts designated to execute SHH on behalf of all child charts
*/}}
{{- define "eric-data-document-database-pg.hkln.executorCharts" -}}
{{- if .Values.global -}}
...
*/}}
{{- define "eric-data-document-database-pg.hkln.executor" -}}
