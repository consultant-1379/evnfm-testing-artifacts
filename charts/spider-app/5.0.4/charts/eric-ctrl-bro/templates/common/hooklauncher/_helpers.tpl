...
*/}}
{{- define "eric-ctrl-bro.hkln.name" -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.job-inventory-secret-name" -}}
{{ printf "%s-inventory" (include "eric-ctrl-bro.hkln.name" .) }}
{{- end }}

{{/*
Name to be used for hooklauncher job container.
*/}}
{{- define "eric-ctrl-bro.hkln.containerName" -}}
{{ printf "hooklauncher" }}
{{- end -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.image-path" }}
...
Create version
*/}}
{{- define "eric-ctrl-bro.hkln.version" -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.helm-labels" }}
chart: {{ .Chart.Name }}
release: "{{ .Release.Name }}"
app: {{ template "eric-ctrl-bro.hkln.name" . }}
app.kubernetes.io/name: {{ template "eric-ctrl-bro.hkln.name" . }}
app.kubernetes.io/version: {{ template "eric-ctrl-bro.hkln.version" . }}
...
Common labels
*/}}
{{- define "eric-ctrl-bro.hkln.labels" -}}
  {{- $helmLabels := include "eric-ctrl-bro.hkln.helm-labels" . | fromYaml -}}
  {{- $globalLabels := (.Values.global).labels -}}
  {{- $serviceLabels := .Values.labels -}}
  {{- $staticLabels := dict -}}
  {{- $_ := set $staticLabels "sidecar.istio.io/inject" "false" -}}
  {{- include "eric-ctrl-bro.mergeLabels" (dict "location" .Template.Name "sources" (list $helmLabels $globalLabels $serviceLabels $staticLabels)) | trim }}
{{- end -}}

{{/*
Create annotation for the product information
*/}}
{{- define "eric-ctrl-bro.hkln.product-info" }}
...
Common annotations
*/}}
{{- define "eric-ctrl-bro.hkln.annotations" -}}
  {{- $productInfoAnn := include "eric-ctrl-bro.hkln.product-info" . | fromYaml -}}
  {{- $globalAnn := (.Values.global).annotations -}}
  {{- $serviceAnn := .Values.annotations -}}
  {{- include "eric-ctrl-bro.mergeAnnotations" (dict "location" .Template.Name "sources" (list $productInfoAnn $globalAnn $serviceAnn)) | trim }}
{{- end -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.cleanup" -}}
{{- $cleanup := "true" -}}
...
Default value: true
*/}}
{{- define "eric-ctrl-bro.hkln.terminateEarlyOnFailure" -}}
...
Default value: 6
*/}}
{{- define "eric-ctrl-bro.hkln.backoffLimit" -}}
{{- $backoffLimit := 6 -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.resources" -}}
  {{- $limitsDict := dict -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.imagePullPolicy" -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.pullSecrets" -}}
{{- $pullSecret := "" -}}
{{- if .Values.global -}}
...
Create tolerations
*/}}
{{- define "eric-ctrl-bro.hkln.tolerations" -}}
  {{- block "eric-ctrl-bro.merge-tolerations" (dict "root" . "podbasename" "hooklauncher") -}}
    {{- with .root -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.priorityClassName" -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.profile-helper" -}}
  {{- $profileType := "" -}}
  {{- $localhostProfile := "" -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.appArmorProfileAnn" -}}
  {{- $result := include "eric-ctrl-bro.hkln.profile-helper" (dict "root" .root "profileKey" "appArmorProfile" "localhostkey" "localhost" "containerName" .containerName) | fromYaml -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.seccompProfile" -}}
{{- if eq .Scope "Pod" -}}
{{- include "eric-ctrl-bro.hkln.profile-helper" (dict "root" .root "profileKey" "seccompProfile" "localhostkey" "Localhost" "containerName" "") -}}
{{- else -}}
{{- include "eric-ctrl-bro.hkln.profile-helper" (dict "root" .root "profileKey" "seccompProfile" "localhostkey" "Localhost" "containerName" .Scope) -}}
{{- end -}}
{{- end -}}

{{- define "eric-ctrl-bro.hkln.securityPolicy.reference" -}}
  {{- if .Values.global -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.securityPolicy.rolename" -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.securityPolicy.rolekind" -}}
{{- $rolekind := "" -}}
  {{- if .Values.global -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.securityPolicy-rolebinding-name" -}}
{{- $rolename := include "eric-ctrl-bro.hkln.securityPolicy.rolename" .shh -}}
{{- $rolecipher := substr 0 1 (include "eric-ctrl-bro.hkln.securityPolicy.rolekind" .shh) | lower -}}
{{- $hklnname := include "eric-ctrl-bro.hkln.name" .top -}}
...
Create timezone
*/}}
{{- define "eric-ctrl-bro.hkln.timezone" -}}
{{- $timezone := "UTC" -}}
{{- if .Values.global -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.nodeSelector" -}}
  {{- $hkln := (.Values.nodeSelector).hooklauncher -}}
  {{- $global := (.Values.global).nodeSelector -}}
  {{- include "eric-ctrl-bro.aggregatedMerge" (dict "context" "nodeSelector" "location" .Template.Name "sources" (list $global $hkln)) -}}
{{- end -}}

{{/*
Integration charts designated to execute SHH on behalf of all child charts
*/}}
{{- define "eric-ctrl-bro.hkln.executorCharts" -}}
{{- if .Values.global -}}
...
*/}}
{{- define "eric-ctrl-bro.hkln.executor" -}}
