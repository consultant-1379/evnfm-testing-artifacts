...
*/}}
{{- define "eric-pm-server.hkln.name" -}}
...
*/}}
{{- define "eric-pm-server.hkln.job-inventory-secret-name" -}}
{{ printf "%s-inventory" (include "eric-pm-server.hkln.name" .) }}
{{- end }}

{{/*
Name to be used for hooklauncher job container.
*/}}
{{- define "eric-pm-server.hkln.containerName" -}}
{{ printf "hooklauncher" }}
{{- end -}}
...
*/}}
{{- define "eric-pm-server.hkln.image-path" }}
...
Create version
*/}}
{{- define "eric-pm-server.hkln.version" -}}
...
*/}}
{{- define "eric-pm-server.hkln.helm-labels" }}
chart: {{ .Chart.Name }}
release: "{{ .Release.Name }}"
app: {{ template "eric-pm-server.hkln.name" . }}
app.kubernetes.io/name: {{ template "eric-pm-server.hkln.name" . }}
app.kubernetes.io/version: {{ template "eric-pm-server.hkln.version" . }}
...
Common labels
*/}}
{{- define "eric-pm-server.hkln.labels" -}}
  {{- $helmLabels := include "eric-pm-server.hkln.helm-labels" . | fromYaml -}}
  {{- $globalLabels := (.Values.global).labels -}}
  {{- $serviceLabels := .Values.labels -}}
  {{- $staticLabels := dict -}}
  {{- $_ := set $staticLabels "sidecar.istio.io/inject" "false" -}}
  {{- include "eric-pm-server.mergeLabels" (dict "location" .Template.Name "sources" (list $helmLabels $globalLabels $serviceLabels $staticLabels)) | trim }}
{{- end -}}

{{/*
Create annotation for the product information
*/}}
{{- define "eric-pm-server.hkln.product-info" }}
...
Common annotations
*/}}
{{- define "eric-pm-server.hkln.annotations" -}}
  {{- $productInfoAnn := include "eric-pm-server.hkln.product-info" . | fromYaml -}}
  {{- $globalAnn := (.Values.global).annotations -}}
  {{- $serviceAnn := .Values.annotations -}}
  {{- include "eric-pm-server.mergeAnnotations" (dict "location" .Template.Name "sources" (list $productInfoAnn $globalAnn $serviceAnn)) | trim }}
{{- end -}}
...
*/}}
{{- define "eric-pm-server.hkln.cleanup" -}}
{{- $cleanup := "true" -}}
...
Default value: true
*/}}
{{- define "eric-pm-server.hkln.terminateEarlyOnFailure" -}}
...
Default value: 6
*/}}
{{- define "eric-pm-server.hkln.backoffLimit" -}}
{{- $backoffLimit := 6 -}}
...
*/}}
{{- define "eric-pm-server.hkln.resources" -}}
  {{- $limitsDict := dict -}}
...
*/}}
{{- define "eric-pm-server.hkln.imagePullPolicy" -}}
...
*/}}
{{- define "eric-pm-server.hkln.pullSecrets" -}}
{{- $pullSecret := "" -}}
{{- if .Values.global -}}
...
Create tolerations
*/}}
{{- define "eric-pm-server.hkln.tolerations" -}}
  {{- block "eric-pm-server.merge-tolerations" (dict "root" . "podbasename" "hooklauncher") -}}
    {{- with .root -}}
...
*/}}
{{- define "eric-pm-server.hkln.priorityClassName" -}}
...
*/}}
{{- define "eric-pm-server.hkln.profile-helper" -}}
  {{- $profileType := "" -}}
  {{- $localhostProfile := "" -}}
...
*/}}
{{- define "eric-pm-server.hkln.appArmorProfileAnn" -}}
  {{- $result := include "eric-pm-server.hkln.profile-helper" (dict "root" .root "profileKey" "appArmorProfile" "localhostkey" "localhost" "containerName" .containerName) | fromYaml -}}
...
*/}}
{{- define "eric-pm-server.hkln.seccompProfile" -}}
{{- if eq .Scope "Pod" -}}
{{- include "eric-pm-server.hkln.profile-helper" (dict "root" .root "profileKey" "seccompProfile" "localhostkey" "Localhost" "containerName" "") -}}
{{- else -}}
{{- include "eric-pm-server.hkln.profile-helper" (dict "root" .root "profileKey" "seccompProfile" "localhostkey" "Localhost" "containerName" .Scope) -}}
{{- end -}}
{{- end -}}

{{- define "eric-pm-server.hkln.securityPolicy.reference" -}}
  {{- if .Values.global -}}
...
*/}}
{{- define "eric-pm-server.hkln.securityPolicy.rolename" -}}
...
*/}}
{{- define "eric-pm-server.hkln.securityPolicy.rolekind" -}}
{{- $rolekind := "" -}}
  {{- if .Values.global -}}
...
*/}}
{{- define "eric-pm-server.hkln.securityPolicy-rolebinding-name" -}}
{{- $rolename := include "eric-pm-server.hkln.securityPolicy.rolename" .shh -}}
{{- $rolecipher := substr 0 1 (include "eric-pm-server.hkln.securityPolicy.rolekind" .shh) | lower -}}
{{- $hklnname := include "eric-pm-server.hkln.name" .top -}}
...
Create timezone
*/}}
{{- define "eric-pm-server.hkln.timezone" -}}
{{- $timezone := "UTC" -}}
{{- if .Values.global -}}
...
*/}}
{{- define "eric-pm-server.hkln.nodeSelector" -}}
  {{- $hkln := (.Values.nodeSelector).hooklauncher -}}
  {{- $global := (.Values.global).nodeSelector -}}
  {{- include "eric-pm-server.aggregatedMerge" (dict "context" "nodeSelector" "location" .Template.Name "sources" (list $global $hkln)) -}}
{{- end -}}

{{/*
Integration charts designated to execute SHH on behalf of all child charts
*/}}
{{- define "eric-pm-server.hkln.executorCharts" -}}
{{- if .Values.global -}}
...
*/}}
{{- define "eric-pm-server.hkln.executor" -}}
