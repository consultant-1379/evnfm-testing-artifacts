...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.name" -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.job-inventory-secret-name" -}}
{{ printf "%s-inventory" (include "eric-pm-bulk-reporter.hkln.name" .) }}
{{- end }}

{{/*
Name to be used for hooklauncher job container.
*/}}
{{- define "eric-pm-bulk-reporter.hkln.containerName" -}}
{{ printf "hooklauncher" }}
{{- end -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.image-path" }}
...
Create version
*/}}
{{- define "eric-pm-bulk-reporter.hkln.version" -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.helm-labels" }}
chart: {{ .Chart.Name }}
release: "{{ .Release.Name }}"
app: {{ template "eric-pm-bulk-reporter.hkln.name" . }}
app.kubernetes.io/name: {{ template "eric-pm-bulk-reporter.hkln.name" . }}
app.kubernetes.io/version: {{ template "eric-pm-bulk-reporter.hkln.version" . }}
...
Common labels
*/}}
{{- define "eric-pm-bulk-reporter.hkln.labels" -}}
  {{- $helmLabels := include "eric-pm-bulk-reporter.hkln.helm-labels" . | fromYaml -}}
  {{- $globalLabels := (.Values.global).labels -}}
  {{- $serviceLabels := .Values.labels -}}
  {{- $staticLabels := dict -}}
  {{- $_ := set $staticLabels "sidecar.istio.io/inject" "false" -}}
  {{- include "eric-pm-bulk-reporter.mergeLabels" (dict "location" .Template.Name "sources" (list $helmLabels $globalLabels $serviceLabels $staticLabels)) | trim }}
{{- end -}}

{{/*
Create annotation for the product information
*/}}
{{- define "eric-pm-bulk-reporter.hkln.product-info" }}
...
Common annotations
*/}}
{{- define "eric-pm-bulk-reporter.hkln.annotations" -}}
  {{- $productInfoAnn := include "eric-pm-bulk-reporter.hkln.product-info" . | fromYaml -}}
  {{- $globalAnn := (.Values.global).annotations -}}
  {{- $serviceAnn := .Values.annotations -}}
  {{- include "eric-pm-bulk-reporter.mergeAnnotations" (dict "location" .Template.Name "sources" (list $productInfoAnn $globalAnn $serviceAnn)) | trim }}
{{- end -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.cleanup" -}}
{{- $cleanup := "true" -}}
...
Default value: true
*/}}
{{- define "eric-pm-bulk-reporter.hkln.terminateEarlyOnFailure" -}}
...
Default value: 6
*/}}
{{- define "eric-pm-bulk-reporter.hkln.backoffLimit" -}}
{{- $backoffLimit := 6 -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.resources" -}}
  {{- $limitsDict := dict -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.imagePullPolicy" -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.pullSecrets" -}}
{{- $pullSecret := "" -}}
{{- if .Values.global -}}
...
Create tolerations
*/}}
{{- define "eric-pm-bulk-reporter.hkln.tolerations" -}}
  {{- block "eric-pm-bulk-reporter.merge-tolerations" (dict "root" . "podbasename" "hooklauncher") -}}
    {{- with .root -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.priorityClassName" -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.profile-helper" -}}
  {{- $profileType := "" -}}
  {{- $localhostProfile := "" -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.appArmorProfileAnn" -}}
  {{- $result := include "eric-pm-bulk-reporter.hkln.profile-helper" (dict "root" .root "profileKey" "appArmorProfile" "localhostkey" "localhost" "containerName" .containerName) | fromYaml -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.seccompProfile" -}}
{{- if eq .Scope "Pod" -}}
{{- include "eric-pm-bulk-reporter.hkln.profile-helper" (dict "root" .root "profileKey" "seccompProfile" "localhostkey" "Localhost" "containerName" "") -}}
{{- else -}}
{{- include "eric-pm-bulk-reporter.hkln.profile-helper" (dict "root" .root "profileKey" "seccompProfile" "localhostkey" "Localhost" "containerName" .Scope) -}}
{{- end -}}
{{- end -}}

{{- define "eric-pm-bulk-reporter.hkln.securityPolicy.reference" -}}
  {{- if .Values.global -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.securityPolicy.rolename" -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.securityPolicy.rolekind" -}}
{{- $rolekind := "" -}}
  {{- if .Values.global -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.securityPolicy-rolebinding-name" -}}
{{- $rolename := include "eric-pm-bulk-reporter.hkln.securityPolicy.rolename" .shh -}}
{{- $rolecipher := substr 0 1 (include "eric-pm-bulk-reporter.hkln.securityPolicy.rolekind" .shh) | lower -}}
{{- $hklnname := include "eric-pm-bulk-reporter.hkln.name" .top -}}
...
Create timezone
*/}}
{{- define "eric-pm-bulk-reporter.hkln.timezone" -}}
{{- $timezone := "UTC" -}}
{{- if .Values.global -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.nodeSelector" -}}
  {{- $hkln := (.Values.nodeSelector).hooklauncher -}}
  {{- $global := (.Values.global).nodeSelector -}}
  {{- include "eric-pm-bulk-reporter.aggregatedMerge" (dict "context" "nodeSelector" "location" .Template.Name "sources" (list $global $hkln)) -}}
{{- end -}}

{{/*
Integration charts designated to execute SHH on behalf of all child charts
*/}}
{{- define "eric-pm-bulk-reporter.hkln.executorCharts" -}}
{{- if .Values.global -}}
...
*/}}
{{- define "eric-pm-bulk-reporter.hkln.executor" -}}
