...
*/}}
{{- define "eric-pm-bulk-reporter.merge-tolerations" -}}
  {{- if (.root.Values.global).tolerations }}
      ...
      {{- range $globalItem := $globalTolerations -}}
        {{- $globalItemId := include "eric-pm-bulk-reporter.merge-tolerations.get-identifier" $globalItem -}}
        {{- range $serviceItem := $serviceTolerations -}}
          {{- $serviceItemId := include "eric-pm-bulk-reporter.merge-tolerations.get-identifier" $serviceItem -}}
      ...
      {{- range $globalItem := $globalTolerations -}}
        ...
        {{- range $matchItem := $matchingItems -}}
          {{- $matchItemId := include "eric-pm-bulk-reporter.merge-tolerations.get-identifier" $matchItem -}}
...
*/}}
{{ define "eric-pm-bulk-reporter.merge-tolerations.get-identifier" }}
  {{- $keyValues := list -}}
