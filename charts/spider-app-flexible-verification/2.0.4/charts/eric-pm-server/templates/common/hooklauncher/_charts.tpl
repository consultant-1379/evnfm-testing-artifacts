{{/* List of subcharts of this chart
     ...
     Uses the template eric-pm-server.hkln.subcharts as an
     ...
     e.g.,
     {{- define "eric-pm-server.hkln.subcharts" -}}
...
*/}}
{{- define "eric-pm-server.hkln.directSubcharts" -}}
{{ block "eric-pm-server.hkln.subcharts" .}}{{ end }}
{{- end -}}

{{- define "eric-pm-server.hkln.hasSHH" -}}
...
{{- define "eric-pm-server.hkln.chartInfo" -}}
{{- if eq "true" (include "eric-pm-server.hkln.hasSHH" .) }}
{{ include "eric-pm-server.hkln.name" . }}:
  jobInventorySecret: {{ include "eric-pm-server.hkln.job-inventory-secret-name" . }}
  version: {{ .Chart.Version | quote }}
{{- end -}}
{{- if eq (include "eric-pm-server.hkln.executor" .) "integration" -}}
{{- range $subChartName, $subChartContext := .Subcharts }}
{{ include "eric-pm-server.hkln.chartInfo" $subChartContext }}
{{- end -}}
{{- end -}}
{{- end -}}
