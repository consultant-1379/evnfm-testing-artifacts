...
`appArmorProfile.eric-pm-alarm-reporter.localhostProfile` | The 'localhost' profile requires a profile name to be provided. The name of the local appArmor profile to be used for eric-pm-alarm-reporter container. | `""`
`appArmorProfile.eric-pm-alarm-reporter.type` | Configuration of AppArmor profile type for eric-pm-alarm-reporter container. Supported values are unconfined, runtime/default, localhost and "". | `""`
`appArmorProfile.eric-pm-br-initcontainer.localhostProfile` | The 'localhost' profile requires a profile name to be provided. The name of the local appArmor profile to be used for eric-pm-br-initcontainer container. | `""`
`appArmorProfile.eric-pm-br-initcontainer.type` | Configuration of AppArmor profile type for eric-pm-br-initcontainer container. Supported values are unconfined, runtime/default, localhost and "". | `""`
`appArmorProfile.eric-pm-bulk-reporter.localhostProfile` | The 'localhost' profile requires a profile name to be provided. The name of the local appArmor profile to be used for eric-pm-bulk-reporter container. | `""`
`appArmorProfile.eric-pm-bulk-reporter.type` | Configuration of AppArmor profile type for eric-pm-bulk-reporter container. Supported values are unconfined, runtime/default, localhost and "". | `""`
`appArmorProfile.eric-pm-sftp.localhostProfile` | The 'localhost' profile requires a profile name to be provided. The name of the local appArmor profile to be used for eric-pm-sftp container. | `""`
`appArmorProfile.eric-pm-sftp.type` | Configuration of AppArmor profile type for eric-pm-sftp container. Supported values are unconfined, runtime/default, localhost and "". | `""`
...
`applicationId.asihHostname` | Hostname of the Application Sys Info Handler service. | `eric-si-application-sys-info-handler`
`applicationId.asihPort` | Port of the Application Sys Info Handler service. | `8080`
`applicationId.asihTlsPort` | Secure port of the Application Sys Info Handler service. | `5000`
`applicationId.enabled` | If set to true, Application Sys Info Handler service will be used to fetch ApplicationId. | `false`
`aumSupport.enabled` | Pre-login legal message generated by AUM. | `false`
`aumSupport.serviceName` | Name of the Admin User Management service | `eric-sec-admin-user-management`
`aumSupport.authFailureDelayDefault` | Default value for Authentication Failure Delay if AUM is not providing the configured value (seconds) | `3`
`enhanceQuery.enabled` | If set to true, the enhance query method will be used. This will only improve the query of measurement-type with collection-method CC. | `false`
`enhanceQuery.lookbackGP` | Number of granularity period the query will lookback if the data is missing at begin or end of granularity period. | `1`
